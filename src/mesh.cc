...
float Mesh::get_bsphere(Vec3 *pos, float *rad) const
{
	if(!bsph_valid) {
		((Mesh*)this)->calc_bsph();
	}
...
void Mesh::calc_bsph()
{
	...
	bsph.pos /= (float)nverts;

	bsph.rad = 0.0f;
	for(unsigned int i=0; i<nverts; i++) {
		v = Vec3(get_attrib(MESH_ATTR_VERTEX, i));
		float dist_sq = length_sq(v - bsph.pos);
		if(dist_sq > bsph.rad) {
			bsph.rad = dist_sq;
		}
	}
	bsph.rad = sqrt(bsph.rad);
